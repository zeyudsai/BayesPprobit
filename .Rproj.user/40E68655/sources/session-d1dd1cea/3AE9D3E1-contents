...
test_that("multi_chain handles invalid inputs", {
  X <- matrix(rnorm(20), nrow = 4, ncol = 5)
  y <- c(1, 0, 1, 0)
  initial_theta <- rep(0, 5)
  ...
  expect_error(
    ...
    "n_sim must be greater than burn_in."
  )
  ...
  expect_error(
    ...
    "initial_theta must have length equal to number of columns in X."
  )
})
